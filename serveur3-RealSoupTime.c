...
int main (int argc, char *argv[]) {
	//char (*data)[RCVSIZE] = (char*)malloc(sizeof(char[ARRSIZE][RCVSIZE]));
	...
	*cwnd = 1;
	...
	if (bind(ctrl_desc, (struct sockaddr*) &adresse_udp, sizeof(adresse_udp)) == -1) {
		perror("Bind ctrl_des failed\n");
		close(ctrl_desc);
		return -1;
	}
	int data_desc=0; //generer un num de port aleatoire
	...
	while (1) {
		...
		if(FD_ISSET(data_desc, &sockTab)){
			printf("entering in the received mode pour %d son", nbre_son);
			int received_size;
			int eof = 0;
			char buffer[RCVSIZE];
		...
		if(FD_ISSET(ctrl_desc, &sockTab)){
			printf("Control msg received by %ld \n", (long)getpid());
			...
			int bool=0;
			for(int i =0; i<10; i++){
				if(adresse_client.sin_addr.s_addr==client_list[i].sin_addr.s_addr && adresse_client.sin_port==client_list[i].sin_port){
					bool=1;
					break;
				}
			}
			if(bool==1){ //probleme de fork
				...
				if(strcmp(Rx,"SYN")==0){ 	
					...
					char Tx[RCVSIZE];
					memset(Tx,0,RCVSIZE);
					int summmm=port+1+nbre_son;
					sprintf(Tx,"SYN-ACK%d",summmm);
					sendto(ctrl_desc,Tx, 11, 0, (struct sockaddr*) &adresse_client, alen);
				}else if(strcmp(Rx,"ACK")==0){
					printf("ACK successfully received !");
					client_list[nbre_son]=adresse_client;
					struct sockaddr_in adresse_server;
					adresse_server.sin_family= AF_INET;
					adresse_server.sin_port= htons(port+nbre_son+1);
					...
					if(fork()==0){
						printf("launching new son on port with %d nbre_son\n", nbre_son);
						
						data_desc=client_desc;
					}else{
						nbre_son+=1;
					}
				}
